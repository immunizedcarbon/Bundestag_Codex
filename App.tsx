...
import { fetchProtocols } from './services/bundestag';
...
const App: React.FC = () => {
  ...
  const [verificationResult, setVerificationResult] = useState<
    | { flash: boolean; pro: boolean; message?: string; timestamp: number }
    | null
  >(null);
  ...
  const handleVerifyGemini = async () => {
    ...
    const result = await verifyGeminiKey(apiKeys.geminiKey);
    setVerificationResult({ ...result, timestamp: Date.now() });
          ...
          {verificationResult && (
            <div
              className={`hidden md:flex items-center gap-2 text-xs px-3 py-1.5 rounded-full border ${
                verificationResult.flash && verificationResult.pro
                  ? 'border-green-200 text-green-700 bg-green-50'
                  : 'border-amber-200 text-amber-700 bg-amber-50'
              }`}
            >
              <ShieldCheck className="w-4 h-4" />
              Gemini {verificationResult.flash && verificationResult.pro ? 'validiert' : 'noch prüfen'}
            </div>
          )}
          <button
